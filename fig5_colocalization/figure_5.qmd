...
df <- fread(here(
  "data/other",
  "Fig5A_0_to_neg40_hub_T_as_anchor_EPI_COLLAPSED.csv"
)) %>%
  mutate(
...
ggdf <- df %>%
  filter(
    neighbor %in% neighbors_sig,
    neighbor %in% neighbors_ord
  ) %>%
  mutate(neighbor = factor(neighbor, levels = neighbors_ord))


plt1 <- ggdf %>%
  ggplot(mapping = aes(x = neighbor, y = anchor, fill = logFC)) +
  geom_tile() +
...
plt2 <- df %>%
  ...
  left_join(
    ...
    by = c("neighbor" = "KNN_celltype_v2")
  ) %>%
  mutate(
    neighbor = factor(neighbor, levels = neighbors_ord),
    KNN_Top = case_when(
      neighbor == "Epi" ~ "Epi",
      neighbor == "Endo-comb" ~ "Strom",
      neighbor == "Macro-comb" ~ "Myeloid",
      neighbor == "Bgc-CD40-comb" ~ "B",
      neighbor %in% c("Tcd4-Treg-comb", "Tplzf-gdlike-comb") ~ "TNKILC",
      TRUE ~ KNN_Top
    )
  ) %>%
  ggplot(mapping = aes(x = neighbor, y = 1, fill = KNN_Top)) +
  geom_tile() +
...
plt <- plt1 + plt2 +
  plot_layout(nrow = 2, heights = c(1, 0.06))
plt
```
